    ...
    if (!empty($data['export_fields'])) {
      ...
      foreach ($query->rows as $i => &$row) {
        foreach ($data['export_fields'] as $field) {
          if (empty($data['filter_language']) && in_array($field, array('name','description','tag','meta_title','meta_h1','meta_description','meta_keyword',/*'seo_keyword',*/'seo_h1','seo_h2','seo_h3','image_title','image_alt'))) {
  ...
  private function setSeoDescription(&$row, $store_id, $language_id) {
    ...
    if (!empty($seoDescription['meta_title'])) {
      $row['meta_title'] = $seoDescription['meta_title'];
        ...
        if (!empty($seoDescription['meta_h1'])) {
      $row['meta_title'] = $seoDescription['meta_h1'];
    }
